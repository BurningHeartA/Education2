...
def Test(GearSet, RatioField, Table, timeFig, timeAx, comboFig, comboAx, comparisonFig, comparisonAx):
    ...
    try:
        ...
        if (ratio > 0.0 and ratio <= 25.0):
            ...
            start_time = round(time.time() * 1000000000)
            Test_Greedy = TestModule.GreedyAlgorithm(GearSet, ratio)
            end_time = round(time.time() * 1000000000)
...
def main():
    ...
    OpenFileButton = ttk.Button(RatioFrame, text="Выбрать файл с набором шестерен",
                                command=lambda: ReadData(FileNameLabel, TestButton, timeFig, timeAx, comboFig, comboAx,
                                                         comparisonFig, comparisonAx, Table))
    ...
    TestButton = ttk.Button(RatioFrame, text="Провести тест", state=tk.DISABLED,
                            command=lambda: Test(GEAR_SET, RatioField, Table, timeFig, timeAx, comboFig, comboAx,
                                                 comparisonFig, comparisonAx))
    ...
    TimeFrame = ttk.LabelFrame(TestFrame, text="Время выполнения(мс)", padding=[5, 5])
